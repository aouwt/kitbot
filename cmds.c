...
COMMAND (kitjoin) {
	...
	IRC_Send (text->chan, KB_RandMessage (Messages.Confirmation));
	IRC_Send (&newch, KB_RandMessage (Messages.Confirmation));
}

COMMAND (kitinsult) {
	IRC_Send (msg->chan, KB_RandMessage (Messages.Insult));
}

COMMAND (kitbeefact) {
	IRC_Send (msg->chan, KB_RandMessage (Messages.BeeFacts));
}
